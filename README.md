...
The fast path handles ~99.99% of inputs using double-double arithmetic. Only the remaining ~0.01% of hard-to-round cases fall through to 128-bit (`MFloat128`) arithmetic.
...
2. **Accurate path**: 128-bit floating-point (`MFloat128`) arithmetic with degree-11/10 polynomials. Two known hard-to-round exceptions handled explicitly.
