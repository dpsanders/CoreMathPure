...
function evalPS(X::MFloat128, X2::MFloat128)
    ...
    Y = (Y + PS[3]) * X2        # degree 5
    Y = (Y + PS[2]) * X2        # degree 3
    Y = (Y + PS[1]) * X         # degree 1
    return Y
end
...
function evalPC(X2::MFloat128)
    ...
    Y = (Y + PC[3]) * X2        # degree 4
    Y = (Y + PC[2]) * X2        # degree 2
    Y = Y + PC[1]               # degree 0
    return Y
end
...
function reduce_dint(X::MFloat128)
    ...
    if e <= 1  # |X| < 2
        u = UInt128(X.hi) * UInt128(T[2])  # T[1] in C (0-indexed)
    ...
    elseif f == 64
        ...
        g = f - 64
        u = UInt128(X.hi) * UInt128(T[i+5])  # T[i+4] in C
        u = u >> 64
...
function reduce_fast(x::Float64)
    if x <= 0x1.921fb54442d17p+2  # x < 2*pi
        ...
        if e <= 1074
            u = UInt128(m) * UInt128(T[2])
            c0 = UInt64(u & typemax(UInt64))
            c1 = UInt64(u >> 64)
            u = UInt128(m) * UInt128(T[1])
            c1 += UInt64(u & typemax(UInt64))
            c2 = UInt64(u >> 64) + UInt64(c1 < UInt64(u & typemax(UInt64)))
            e = 1075 - e
        else
            i = (e - 1138 + 63) ÷ 64
            u = UInt128(m) * UInt128(T[i+3])
            c0 = UInt64(u & typemax(UInt64))
            c1 = UInt64(u >> 64)
            u = UInt128(m) * UInt128(T[i+2])
            c1 += UInt64(u & typemax(UInt64))
            c2 = UInt64(u >> 64) + UInt64(c1 < UInt64(u & typemax(UInt64)))
            u = UInt128(m) * UInt128(T[i+1])
...
function sin_fast(x::Float64)
    ...
    if is_sin
        sh, sl = s_mul(sgn_val * SC[i+1][3], sh, sl)
        ch, cl = s_mul(sgn_val * SC[i+1][2], ch, cl)
        h, l2 = fast_two_sum(ch, sh)
        l2 += sl + cl
        err = 0x1.55p-69
    else
        ch, cl = s_mul(sgn_val * SC[i+1][3], ch, cl)
        sh, sl = s_mul(sgn_val * SC[i+1][2], sh, sl)
        h, l2 = fast_two_sum(ch, -sh)
        l2 += cl - sl
        err = 0x1.81p-69
    end
