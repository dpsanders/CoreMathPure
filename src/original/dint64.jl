...
@inline get_r(d::DInt64) = (UInt128(d.hi) << 64) | UInt128(d.lo)

# Create DInt64 from 128-bit mantissa r, preserving ex and sgn
@inline function from_r(r::UInt128, ex::Int64, sgn::UInt64)
    DInt64(UInt64(r >> 64), UInt64(r & typemax(UInt64)), ex, sgn)
end

@inline dint_zero_p(a::DInt64) = a.hi == 0
...
function cmp_dint_abs(a::DInt64, b::DInt64)
    dint_zero_p(a) && return dint_zero_p(b) ? 0 : -1
    dint_zero_p(b) && return 1
    c1 = cmp_i64(a.ex, b.ex)
    return c1 != 0 ? c1 : cmp_u128(get_r(a), get_r(b))
end
...
# Convert a non-zero double to DInt64
function dint_fromd(b::Float64)
...
# Add two DInt64 values, with error bounded by 2 ulps (ulp_128)
function add_dint(a::DInt64, b::DInt64)
    if (a.hi | a.lo) == 0
        return b
    end
    ...
    return from_r(C, rex, sgn)
end

# Multiply two DInt64 numbers, with error bounded by 6 ulps
function mul_dint(a::DInt64, b::DInt64)
    bh = UInt128(b.hi)
    bl = UInt128(b.lo)
    ...
    # 128-bit product of high terms
    r = UInt128(a.hi) * bh

    # Add upper halves of middle terms
    r += (m1 >> 64) + (m2 >> 64)

    # Ensure hi starts with a 1
    hi = UInt64(r >> 64)
    ex_bit = hi >> 63
    r = r << (1 - ex_bit)
...
function subnormalize_dint(a::DInt64)
    a.ex > -1023 && return a
    ...
    return DInt64(new_hi, new_lo, a.ex, a.sgn)
end

# Convert DInt64 to Float64 (round to nearest)
function dint_tod(a::DInt64)
    a = subnormalize_dint(a)

    # Construct base value in [1, 2)
